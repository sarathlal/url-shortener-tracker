...
1. **Creating a New Redirect:**
    - Navigate to the 'URLs' menu in the WordPress admin panel.
    - Fill out the form with the original URL and the redirect URL.
    - Click 'Add URL' to save the new redirect.

2. **Managing Existing URLs:**
    - The list of existing URLs will be displayed in the 'URLs' menu.
    - Use the 'Edit' link to modify an existing URL.
    - Use the 'Delete' link to remove a URL.
    - Select multiple URLs using the checkboxes and apply bulk actions for efficient management.
...
Go to the 'URLs' menu in the WordPress admin panel and fill out the form to create a new redirect.

= Can I track the number of clicks on each URL? =

Yes, the plugin provides click tracking for each URL.

= How do I bulk delete URLs? =

Select multiple URLs using the checkboxes and choose 'Delete' from the bulk actions dropdown, then click 'Apply'.

= Can I change the number of URLs displayed per page? =

Yes, you can set the number of URLs displayed per page using the input box in the URL list screen.

== Screenshots ==

1. **Admin URL Management Screen:** Easily manage your URL redirects and track clicks.
